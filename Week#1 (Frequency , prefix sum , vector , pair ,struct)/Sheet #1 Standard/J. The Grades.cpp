/* Depression i snot a side effect of illness,
...
// --------------------- Main Function (solve )------------------------------- //
// -------------------------------------------------------------------------- //
const ll OO = LLONG_MAX, N = 1e5 + 5, mod = 1e9 + 7;

void solve() {
    ll n; cin >> n;

    map <ll,vector<string>> r;
    map<string,vector<ll>> d;
    vl deg;

    F(i,0,n){
        string name; ll T,A,M,S,E;
        cin >> name >>A>>M>>S>>E;

        T= A+M+S+E; deg.pb(T);
        r[T].pb(name);
        d[name].pb(A);d[name].pb(M);d[name].pb(S);d[name].pb(E);
    }
    ...
    sl deg1; F(i,0,sz(deg)) deg1.insert(deg[i]);
    deg.clear(); for(const auto &it : deg1) deg.pb(it);
    SORT(deg); REV(deg);
